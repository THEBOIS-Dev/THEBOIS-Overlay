  ...
  <div class="flex items-center justify-between py-2 gap-4">
    ...
    <div class="flex items-center gap-1.5 no-drag shrink-0">
      <input
        ...
        :style="capturing ? 'border-color: var(--color-accent); box-shadow: 0 0 0 2px var(--color-accent-glow);' : ''"
        @click="startCapture"
        @keydown.stop="onKey"
        @blur="stopCapture"
      />
...
function startCapture() { capturing.value = true }
function stopCapture()  { capturing.value = false }
