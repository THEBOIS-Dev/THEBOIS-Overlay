    ...
    >
        ...
        <div class="flex items-center gap-2 min-w-0">
          ...
          >
            <template v-if="player.nicked && player.name !== player.realName">
              ...
              <span style="color: rgba(255,255,255,0.25); margin: 0 3px; font-size: 0.7rem;">→</span>
          ...
          <span
            v-if="player.nicked"
            class="tag shrink-0"
            style="background: rgba(253,230,138,0.12); color: var(--color-nick); border: 1px solid rgba(253,230,138,0.25);"
          >NICK</span>
      ...
      <template v-else>
        <span
          ...
          style="font-family: var(--font-mono); font-size: 0.88rem; font-weight: 500; letter-spacing: 0.02em;"
        >{{ cellValue(col) }}</span>
      </template>
    </td>
...
import {
  Column, COLUMNS, statColor, fmt,
  getTopRankDisplay, playerNameColor, isStaff,
  type Player,
} from '@renderer/types'
...
const errorLabel = computed(() => {
  switch (props.player.error) {
    case 'not_found':    return 'N/A'
    case 'rate_limited': return '429'
    case 'network':      return 'ERR'
    default:             return '?'
  }
})
...
const errorColor = computed(() => {
  switch (props.player.error) {
    case 'not_found':    return 'var(--color-ink-3)'
    case 'rate_limited': return '#f97316'
    default:             return 'var(--color-bad)'
  }
})
