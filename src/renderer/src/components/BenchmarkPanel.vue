...
import { ref, computed, nextTick, onUnmounted, markRaw } from 'vue';
...
function stat(v: number): PikaStatGroup {
  return { entries: [{ value: v }] };
}

function makeProfile(rankName: string, level: number): PikaProfile {
  return {
...
const MOCK_PLAYERS: Player[] = [
  {
    ...
    team: 'Red',
    teamColor: '#ff4455',
    profile: makeProfile('srmod', 200),
    stats: makeStats(82000, 1200, 15000, 800, 120000, 9000),
  },
  {
    name: 'Dream',
    realName: 'Dream',
    ...
    team: 'Red',
    teamColor: '#ff4455',
    profile: makeProfile('moderator', 150),
    stats: makeStats(45000, 3200, 8000, 1200, 70000, 12000),
  },
  {
    name: 'Skeppy',
    realName: 'Skeppy',
    ...
    team: 'Blue',
    teamColor: '#4488ff',
    profile: makeProfile('helper', 100),
    stats: makeStats(12000, 8000, 2000, 2500, 25000, 18000),
  },
  {
    name: 'BadBoyHalo',
    realName: 'BadBoyHalo',
    ...
    team: 'Blue',
    teamColor: '#4488ff',
    profile: makeProfile('trial', 75),
    stats: makeStats(5000, 6000, 800, 1800, 9000, 14000),
  },
  {
    name: 'xNestorio',
    realName: 'xNestorio',
    ...
    team: 'Green',
    teamColor: '#44dd66',
    profile: makeProfile('youtuber', 120),
    stats: makeStats(30000, 4000, 6000, 2000, 48000, 15000),
  },
  {
    name: 'n1ck3d',
    realName: 'GoodPlayer',
    ...
    team: 'Green',
    teamColor: '#44dd66',
    profile: makeProfile('games4', 88),
    stats: makeStats(18000, 5000, 3200, 1500, 32000, 13000),
  },
  {
    name: 'CrystalLegend',
    ...
    team: 'Yellow',
    teamColor: '#ffdd33',
    profile: makeProfile('games3', 60),
    stats: makeStats(4000, 7000, 500, 2200, 7000, 16000),
  },
  {
    name: 'NovicePlayer99',
    ...
    teamColor: '#ffdd33',
    profile: null,
    stats: makeStats(120, 900, 10, 400, 300, 2200),
  },
  {
    name: 'ShadowBlade',
    ...
    team: null,
    teamColor: null,
    profile: makeProfile('admin', 180),
    stats: makeStats(60000, 2100, 11000, 900, 90000, 8000),
  },
  {
    name: 'PrivateStats',
  ...
  },
  {
    ...
    team: null,
    teamColor: null,
    profile: makeProfile('games1', 30),
    stats: makeStats(800, 1200, 120, 380, 2200, 4100),
  },
  {
    name: 'NetworkErr',
    realName: 'NetworkErr',
...
function addPhase(id: string, label: string): PhaseResult {
  const p: PhaseResult = { id, label, status: 'running', metrics: [], durationMs: 0 };
  phases.value.push(p);
  return p;
}

function finishPhase(p: PhaseResult, startTime: number): void {
  p.durationMs = performance.now() - startTime;
  p.status = 'done';
}
...
async function runFrameTimingTest(): Promise<PhaseResult> {
  ...
  const jankFrames = frames.filter((f) => f > 33.33).length;
...
async function runLongTaskTest(): Promise<PhaseResult> {
  ...
  players.players = markRaw([...MOCK_PLAYERS]);
  await waitFrames(60);
  ...
  const totalDur = longTasks.reduce((s, e) => s + e.duration, 0);
  const maxTask = longTasks.length ? Math.max(...longTasks.map((e) => e.duration)) : 0;
...
async function runPlayerRenderTest(): Promise<PhaseResult> {
    ...
    players.players[i] = {
      ...
      stats: p.stats
        ? {
            ...p.stats,
            'Final kills': stat(
              ((p.stats['Final kills']?.entries?.[0]?.value as number) ?? 0) + 1,
            ),
          }
        : p.stats,
    };
  ...
  players.players = markRaw([...MOCK_PLAYERS].map((p) => ({ ...p, loading: true })));
  await waitFrames(1);
...
async function runAnimationStressTest(): Promise<PhaseResult> {
  ...
  const p95 = percentile(sorted, 95);
  const p99 = percentile(sorted, 99);
  const dropped60 = frames.filter((f) => f > 16.67).length;
  const dropped30 = frames.filter((f) => f > 33.33).length;
  const droppedPct = (dropped60 / frames.length) * 100;
  const nickedCount = MOCK_PLAYERS.filter((p) => p.nicked).length;
...
async function runReactivityTest(): Promise<PhaseResult> {
  ...
  for (let r = 0; r < ROUNDS; r++) {
    ...
    const idx = r % players.players.length;
    const p = players.players[idx];
    players.players[idx] = {
      ...p,
      stats: p.stats ? { ...p.stats, 'Final kills': stat(r) } : p.stats,
    };
    await nextTick();
  ...
  const mean = roundTimes.reduce((s, v) => s + v, 0) / roundTimes.length;
  const p95 = percentile(sorted, 95);
  const p99 = percentile(sorted, 99);
  ...
  players.players = markRaw(
    players.players.map((p, i) => ({
      ...p,
      stats: p.stats ? { ...p.stats, Wins: stat(i * 100) } : p.stats,
    })),
  );
  await nextTick();
...
async function runAvatarCacheTest(): Promise<PhaseResult> {
  ...
  const NAMES = MOCK_PLAYERS.filter((p) => !p.loading && !p.error).map((p) => p.realName);
  ...
  for (let i = 0; i < TEST_ROUNDS; i++) {
    ...
    localStorageTimes.push(performance.now() - t);
  }

  const memCache = new Map<string, string>();
  NAMES.forEach((n) => memCache.set(`mchead_${n.toLowerCase()}`, `data:fake/${n}`));
  ...
  for (let i = 0; i < TEST_ROUNDS; i++) {
    ...
    const t = performance.now();
    memCache.get(key);
    memTimes.push(performance.now() - t);
  }

  const jsonSizes: number[] = [];
  for (const n of NAMES) {
    const raw = localStorage.getItem(`mchead_${n.toLowerCase()}`);
    if (raw) jsonSizes.push(raw.length);
  }
  const avgJsonSize = jsonSizes.length
    ? jsonSizes.reduce((s, v) => s + v, 0) / jsonSizes.length
    : 0;

  const lsMean = localStorageTimes.reduce((s, v) => s + v, 0) / localStorageTimes.length;
  const memMean = memTimes.reduce((s, v) => s + v, 0) / memTimes.length;
...
async function runMemoryTest(): Promise<PhaseResult> {
  ...
  const heapPerPlayer = heapDelta / MOCK_PLAYERS.length;
...
async function runScrollTest(): Promise<PhaseResult> {
  ...
  for (let pass = 0; pass < PASSES; pass++) {
    ...
    for (let s = 0; s <= steps; s++) {
      const t = performance.now();
      scrollEl.scrollTop = (maxScroll / steps) * s;
      await waitFrames(1);
      scrollTimes.push(performance.now() - t);
    }
  ...
  const mean = scrollTimes.reduce((s, v) => s + v, 0) / scrollTimes.length;
...
async function runAll(): Promise<void> {
  ...
  try {
    ...
    const allMetrics = phases.value.flatMap((p) => p.metrics);
    const badCount = allMetrics.filter((m) => m.rating === 'bad').length;
    const warnCount = allMetrics.filter((m) => m.rating === 'warn').length;
    overallRating.value =
...
const totalDuration = computed(() => phases.value.reduce((s, p) => s + p.durationMs, 0));
</script>
      ...
      <header class="bench-header">
        <div class="bench-header-left">
          <Activity
            ...
            class="bench-icon"
          />
          <h1 class="bench-title">Performance Benchmark</h1>
        </div>
...
.bench-run-btn {
  ...
  transition:
    opacity 0.2s,
    box-shadow 0.2s;
  box-shadow: 0 2px 12px rgba(109, 140, 235, 0.25);
  letter-spacing: 0.01em;
}
...
.bench-run-btn:not(:disabled):hover {
  opacity: 0.9;
}
...
.phase-card {
  background: var(--color-surface-1, #12161e);
  border-radius: 10px;
