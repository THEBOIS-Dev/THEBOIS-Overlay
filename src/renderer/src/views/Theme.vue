  ...
  <div class="flex h-full overflow-hidden" style="pointer-events: all;">
    ...
    <div class="flex-1 overflow-y-auto p-3 flex flex-col gap-3 themed-scroll">
      ...
      <template v-if="activeTab === 'background'">
          ...
          <div class="card divide-subtle">
            ...
            <div class="flex items-center gap-3 px-3 py-2.5 no-drag">
              <label class="cursor-pointer relative shrink-0">
                ...
                />
                <input type="color" :value="solidHex" class="absolute opacity-0 w-0 h-0 pointer-events-none"
            ...
            <div class="px-3 py-2.5 no-drag">
              ...
              <input type="range" min="0" max="1" step="0.01" :value="theme.opacity"
        ...
        <template v-if="theme.bgType === 'gradient'">
          ...
          <div class="card px-3 py-2.5">
            <span class="text-xs font-semibold uppercase tracking-widest mb-2 block" style="color: var(--color-ink-3)">Direction</span>
            <div class="flex flex-wrap gap-1 no-drag">
              <button
                v-for="d in DIRECTIONS" :key="d.value"
                class="w-8 h-7 rounded text-sm transition-all"
                :style="theme.bgGradientDir === d.value
          ...
          <div class="card divide-subtle">
            <div
              v-for="(stop, i) in theme.bgGradientStops" :key="i"
            ...
            >
              <label class="cursor-pointer relative shrink-0">
                ...
                />
                <input type="color" :value="stop.color" class="absolute opacity-0 w-0 h-0 pointer-events-none"
              ...
              />
              <span class="text-xs font-mono tabular-nums" style="color: var(--color-ink-3); min-width: 30px; text-align: right;">
                {{ stop.position }}%
              </span>
              <button
            ...
            <div class="px-3 py-2 no-drag">
              <button
                ...
                style="background: rgba(124,58,237,0.1); color: #b89aff; border: 1px solid rgba(124,58,237,0.22);"
                @click="addStop"
              >+ Add Stop</button>
            </div>
          </div>
        ...
        <template v-if="theme.bgType === 'image'">
          <div class="card divide-subtle">
            ...
            <div class="px-3 py-2.5 no-drag">
              <span class="text-xs font-medium block mb-2" style="color: var(--color-ink-2)">Background Image</span>
              <div class="flex flex-col gap-2">
                <button
                  class="flex items-center justify-center gap-2 rounded-lg text-xs py-2 px-3 transition-all"
                  style="background: rgba(124,58,237,0.12); color: #b89aff; border: 1px solid rgba(124,58,237,0.28);"
                ...
                <div v-if="theme.bgImageUrl" class="flex items-center gap-2 px-2 py-1.5 rounded" style="background: rgba(255,255,255,0.04); border: 1px solid var(--color-border);">
                  <svg viewBox="0 0 24 24" width="10" height="10" fill="currentColor" style="color: var(--color-good); flex-shrink: 0;">
                    <path d="M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z"/>
                  </svg>
                  <span class="text-xs truncate flex-1" style="color: var(--color-ink-3); font-family: var(--font-mono);">
                    {{ imageFilename }}
                  </span>
                  <button
            ...
            <div class="px-3 py-2.5 no-drag">
              <div class="flex justify-between mb-1.5">
                <span class="text-xs font-medium" style="color: var(--color-ink-2)">Image opacity</span>
                <span class="text-xs font-mono tabular-nums" style="color: var(--color-accent-light)">
                  {{ Math.round(theme.bgImageOpacity * 100) }}%
                </span>
              </div>
              <input type="range" min="0" max="1" step="0.01" :value="theme.bgImageOpacity"
                class="themed-range w-full"
                @input="theme.bgImageOpacity = parseFloat(($event.target as HTMLInputElement).value)"
                @change="theme.bgImageOpacity = parseFloat(($event.target as HTMLInputElement).value)"
              />
            </div>

            <div class="px-3 py-2.5 no-drag">
              <div class="flex justify-between mb-1.5">
                <span class="text-xs font-medium" style="color: var(--color-ink-2)">Overlay opacity</span>
                <span class="text-xs font-mono tabular-nums" style="color: var(--color-accent-light)">
                  {{ Math.round(theme.opacity * 100) }}%
                </span>
              </div>
        ...
        <div>
          <div class="flex items-center gap-2 mb-1.5">
            <span class="text-xs font-semibold uppercase tracking-widest" style="color: var(--color-ink-3)">Preview</span>
            ...
            <div class="absolute inset-0 flex items-center px-3 gap-3">
              ...
              <div class="flex flex-col gap-1 min-w-0 flex-1">
                <div class="flex gap-3 items-center">
                  <span class="text-xs font-bold" style="color: var(--color-ink-1)">PlayerName</span>
                  <span class="text-xs font-mono" style="color: var(--color-good)">6.50</span>
                  <span class="text-xs font-mono" style="color: var(--color-warn, #fbbf24)">2.30</span>
                  <span class="text-xs font-mono" style="color: var(--color-accent-light)">1,204</span>
                </div>
      ...
      <template v-if="activeTab === 'colors'">
        ...
        <div>
          <div class="flex items-center gap-2 mb-1.5">
            <span class="text-xs font-semibold uppercase tracking-widest" style="color: var(--color-ink-3)">Interface</span>
            <div class="flex-1 h-px" style="background: var(--color-border)" />
          </div>
          <div class="card divide-subtle">
            <ColorRow
              v-for="c in UI_COLORS" :key="c.key"
              :label="c.label"
        ...
        <div>
          <div class="flex items-center gap-2 mb-1.5">
            <span class="text-xs font-semibold uppercase tracking-widest" style="color: var(--color-ink-3)">Rank Colors</span>
            <div class="flex-1 h-px" style="background: var(--color-border)" />
          </div>
          <div class="card divide-subtle">
            <ColorRow
              v-for="r in RANK_COLORS" :key="r.key"
              :label="r.label"
...
const BG_TYPES = [
  ...
  { id: 'gradient', label: 'Gradient', icon: 'M11 9h2v2h-2zm-2 2h2v2H9zm4 0h2v2h-2zm2-2h2v2h-2zM7 9h2v2H7zm12-6H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2V5c0-1.1-.9-2-2-2zm0 16H5V5h14v14zM9 13h2v2H9zm-2 0h2v2H7zm4 0h2v2h-2zm2 0h2v2h-2zm2 0h2v2h-2z' },
  { id: 'image',    label: 'Image',    icon: 'M21 19V5c0-1.1-.9-2-2-2H5c-1.1 0-2 .9-2 2v14c0 1.1.9 2 2 2h14c1.1 0 2-.9 2-2zM8.5 13.5l2.5 3.01L14.5 12l4.5 6H5l3.5-4.5z' },
]
