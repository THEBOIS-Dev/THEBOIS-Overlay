    ...
    <div class="flex-1 overflow-y-auto p-3 flex flex-col gap-4 themed-scroll">
      ...
      <div v-if="activeTab === 'appearance'">
        <Section title="Appearance">
          ...
          />
          <ToggleSetting label="Rounded corners" :value="config.roundedCorners" @update="config.roundedCorners = $event" />
          <ToggleSetting label="Text shadow"     :value="config.textShadow"     @update="config.textShadow = $event" />
          ...
          />
          <p v-if="config.integratedMode" class="text-xs px-1 pb-1" style="color: var(--color-ink-3); line-height: 1.5;">
            Footer &amp; column labels hidden, background fully transparent. Hover the header to reveal controls.
          </p>
        </Section>
      </div>
      ...
      <div v-if="activeTab === 'columns'">
        <Section title="Active Columns" description="Toggle columns on/off. Use ↑↓ arrows to reorder.">
          <div v-if="config.activeColumns && config.activeColumns.length" class="flex flex-col gap-1.5 no-drag py-1">
            <!-- Active columns with reorder controls -->
            <div class="mb-1">
              <p class="text-xs mb-1.5" style="color: var(--color-ink-3); font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.06em;">Active (in order)</p>
              <div class="flex flex-col gap-1">
                <div
                  v-for="(col, idx) in config.activeColumns"
                  :key="col"
                  class="flex items-center justify-between px-2.5 py-1.5 rounded-lg"
                  style="background: var(--color-accent-dim); border: 1px solid rgba(124,58,237,0.3);"
                >
                  <div class="flex items-center gap-2">
                    <div class="w-1.5 h-1.5 rounded-full" style="background: var(--color-accent)" />
                    <span class="text-xs font-medium" style="color: var(--color-accent-light)">{{ COLUMNS[col].label }}</span>
                  </div>
            ...
            <div>
              <p class="text-xs mb-1.5" style="color: var(--color-ink-3); font-size: 0.7rem; text-transform: uppercase; letter-spacing: 0.06em;">Available</p>
              <div class="flex flex-col gap-1">
                <div
                ...
                >
                  <div class="flex items-center gap-2">
                    <div class="w-1.5 h-1.5 rounded-full" style="background: rgba(255,255,255,0.15)" />
                    <span class="text-xs font-medium" style="color: var(--color-ink-3)">{{ COLUMNS[col].label }}</span>
                  </div>
      ...
      <div v-if="activeTab === 'sorting'">
        <Section title="Sort By">
          <div v-if="SORTABLE_COLUMNS.length" class="flex flex-col gap-1.5 no-drag py-1">
            ...
            >
              <div
                ...
                :style="{ background: config.sortBy === col ? 'var(--color-accent)' : 'rgba(255,255,255,0.15)' }"
              />
              {{ COLUMNS[col].label }}
            </div>
          </div>
          ...
          <ToggleSetting label="Sort ascending" :value="config.sortAscending" @update="config.sortAscending = $event" />
        </Section>
      </div>

      <!-- Shortcuts -->
      <div v-if="activeTab === 'shortcuts'">
        <Section title="Keyboard Shortcuts" description="Click a field then press your desired key combo.">
          <ShortcutInput label="Minimize / Restore" :value="config.shortcutMinimize"     @update="onShortcutChange('shortcutMinimize', $event)" />
          <ShortcutInput label="Clear Players"      :value="config.shortcutClearPlayers"  @update="onShortcutChange('shortcutClearPlayers', $event)" />
        </Section>
      </div>
      ...
      <Section title="Discord Rich Presence">
        <div class="flex flex-col gap-3">
          ...
          <p class="text-[11px] px-1" style="color: var(--color-ink-3); line-height: 1.5;">
            Shows your status in Discord. Active when log is streaming, Idle after 15 seconds of inactivity.
          </p>
        </div>
      </Section>
      ...
      <div v-if="activeTab === 'advanced'">
        <Section title="Auto-detection">
          <ToggleSetting label="Auto-add on join"         :value="config.autoAddPlayers"              @update="config.autoAddPlayers = $event" />
          <ToggleSetting label="Clear list on /who"       :value="config.autoRemoveAllOnWho"           @update="config.autoRemoveAllOnWho = $event" />
          <ToggleSetting label="Remove on final kill"     :value="config.autoRemoveFinalDeath"         @update="config.autoRemoveFinalDeath = $event" />
          <ToggleSetting label="Remove on quit"           :value="config.autoRemoveOnQuit"             @update="config.autoRemoveOnQuit = $event" />
          <ToggleSetting label="Missing players warning"  :value="config.missingPlayersWarning"        @update="config.missingPlayersWarning = $event" />
        </Section>
        ...
        <Section title="Updates" class="mt-1">
          ...
          />
          <p class="text-[11px] px-1" style="color: var(--color-ink-3); line-height: 1.5;">
            Automatically checks for updates from GitHub on launch. Downloads and installs on next exit.
          </p>
          <div class="flex items-center justify-between py-2">
        ...
        <Section title="Danger Zone" class="mt-1">
          <div class="flex items-center justify-between py-2">
            <span class="text-xs" style="color: var(--color-ink-2)">Reset all settings to defaults</span>
            <button
              class="btn px-3 py-1.5 rounded-lg text-xs no-drag"
...
type UpdateStatus = 'idle' | 'dev' | 'checking' | 'up-to-date' | 'available' | 'downloading' | 'downloaded' | 'error'
...
const updateStatusText = computed(() => {
  switch (updateStatus.value) {
    case 'dev':         return 'Updates disabled in dev mode'
    case 'checking':    return 'Checking for updates...'
    case 'up-to-date':  return 'You are on the latest version'
    case 'available':   return `Update available: v${updateVersion.value}`
    case 'downloading': return `Downloading... ${updatePercent.value}%`
    case 'downloaded':  return `v${updateVersion.value} downloaded`
    case 'error':       return `Update error: ${updateError.value}`
...
const TABS = [
  { id: 'appearance', label: 'Appearance', iconPath: 'M12 3c-4.97 0-9 4.03-9 9s4.03 9 9 9c.83 0 1.5-.67 1.5-1.5 0-.39-.15-.74-.39-1.01-.23-.26-.38-.61-.38-.99 0-.83.67-1.5 1.5-1.5H16c2.76 0 5-2.24 5-5 0-4.42-4.03-8-9-8zm-5.5 9c-.83 0-1.5-.67-1.5-1.5S5.67 9 6.5 9 8 9.67 8 10.5 7.33 12 6.5 12zm3-4C8.67 8 8 7.33 8 6.5S8.67 5 9.5 5s1.5.67 1.5 1.5S10.33 8 9.5 8zm5 0c-.83 0-1.5-.67-1.5-1.5S13.67 5 14.5 5s1.5.67 1.5 1.5S15.33 8 14.5 8zm3 4c-.83 0-1.5-.67-1.5-1.5S16.67 9 17.5 9s1.5.67 1.5 1.5-.67 1.5-1.5 1.5z' },
  { id: 'columns',    label: 'Columns',    iconPath: 'M3 5h8V3H3v2zm0 8h8v-2H3v2zm0 8h8v-2H3v2zm10-8h8v-2h-8v2zm0 8h8v-2h-8v2zm0-16v2h8V5h-8z' },
  { id: 'sorting',    label: 'Sorting',    iconPath: 'M3 18h6v-2H3v2zM3 6v2h18V6H3zm0 7h12v-2H3v2z' },
  { id: 'shortcuts',  label: 'Shortcuts',  iconPath: 'M20 5H4c-1.1 0-1.99.9-1.99 2L2 17c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V7c0-1.1-.9-2-2-2zm-9 3h2v2h-2V8zm0 3h2v2h-2v-2zM8 8h2v2H8V8zm0 3h2v2H8v-2zm-1 5H5v-2h2v2zm9 0H8v-2h8v2zm0-3h-2v-2h2v2zm0-3h-2V8h2v2zm3 6h-2v-2h2v2zm0-3h-2v-2h2v2zm0-3h-2V8h2v2z' },
  { id: 'advanced',   label: 'Advanced',   iconPath: 'M19.14 12.94c.04-.3.06-.61.06-.94 0-.32-.02-.64-.07-.94l2.03-1.58c.18-.14.23-.41.12-.61l-1.92-3.32c-.12-.22-.37-.29-.59-.22l-2.39.96c-.5-.38-1.03-.7-1.62-.94l-.36-2.54c-.04-.24-.24-.41-.48-.41h-3.84c-.24 0-.43.17-.47.41l-.36 2.54c-.59.24-1.13.57-1.62.94l-2.39-.96c-.22-.08-.47 0-.59.22L2.74 8.87c-.12.21-.08.47.12.61l2.03 1.58c-.05.3-.09.63-.09.94s.02.64.07.94l-2.03 1.58c-.18.14-.23.41-.12.61l1.92 3.32c.12.22.37.29.59.22l2.39-.96c.5.38 1.03.7 1.62.94l.36 2.54c.05.24.24.41.48.41h3.84c.24 0 .44-.17.47-.41l.36-2.54c.59-.24 1.13-.56 1.62-.94l2.39.96c.22.08.47 0 .59-.22l1.92-3.32c.12-.22.07-.47-.12-.61l-2.01-1.58zM12 15.6c-1.98 0-3.6-1.62-3.6-3.6s1.62-3.6 3.6-3.6 3.6 1.62 3.6 3.6-1.62 3.6-3.6 3.6z' },
]
...
async function onShortcutChange(key: 'shortcutMinimize' | 'shortcutClearPlayers', value: string): Promise<void> {
  config[key] = value
