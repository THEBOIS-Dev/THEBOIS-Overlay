 ...
 * This means color codes arrive as "\uFFFDr" instead of "§r". This handles both.
 */
...
const PATTERNS = {
  ...
  jartexJoin: new RegExp(
    `^BedWars \\? (${MC_NAME}) has joined the game! \\(\\d+/\\d+\\)$`,
  ),

  jartexQuit: new RegExp(`^BedWars \\? (${MC_NAME}) has left the game! \\(\\d+/\\d+\\)$`),

  whoPlain:
    /^((?:\[[A-Z0-9+_§]+?] )?[A-Za-z0-9_]+)(?:,\s*(?:\[[A-Z0-9+_§]+?] )?[A-Za-z0-9_]+)+$/,
...
export function parseLine(raw: string): LogEvent | null {
  ...
  const joinM = msg.match(PATTERNS.join) ?? msg.match(PATTERNS.jartexJoin);
  if (joinM) return { type: 'join', name: joinM[1] };

  const quitM =
    msg.match(PATTERNS.quit) ??
    msg.match(PATTERNS.quitDc) ??
    msg.match(PATTERNS.jartexQuit);
